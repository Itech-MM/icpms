...
public class AuthenticationService implements UserDetailsService {
	...
	private Set<GrantedAuthority> buildAuthorities(User user) {
		...
		if (user.getRole() != null && user.getRole().getCode() != null) {
			...
			List<MenuRoleAccess> menuAccesses = menuRoleAccessRepository.findByRoleIdWithMenu(user.getRole().getId());
			...
			for (MenuRoleAccess access : menuAccesses) {
				if (access.getMenu() != null && access.getMenu().getCode() != null) {
					...
					if (Boolean.TRUE.equals(access.getCanView())) {
						authorities
								.add(new SimpleGrantedAuthority(MenuPermission.VIEW.getAuthority() + "_" + menuCode));
					}
					if (Boolean.TRUE.equals(access.getCanAccess())) {
						authorities
								.add(new SimpleGrantedAuthority(MenuPermission.ACCESS.getAuthority() + "_" + menuCode));
					}
					if (Boolean.TRUE.equals(access.getCanEdit())) {
						authorities
								.add(new SimpleGrantedAuthority(MenuPermission.EDIT.getAuthority() + "_" + menuCode));
					}
					if (Boolean.TRUE.equals(access.getCanDelete())) {
						authorities
								.add(new SimpleGrantedAuthority(MenuPermission.DELETE.getAuthority() + "_" + menuCode));
					}
				}
			}
		}
